...
from src.services.usecases.graph import build_differ_and_save, build_timecompare_graphs_and_save, build_mean_graphs_and_save, \
    build_detailed_graphs_and_save
...
def export_diff_graphs(to_path: str) -> None:
    """
    Выгружает diff- графики в папку :param to_path
    """
    stats = get_measurements_stats()
    build_differ_and_save(stats, to_path)


def export_time_compare_graphs(path: str) -> None:
    groups = get_all_groups()
    build_timecompare_graphs_and_save(groups, path)
...
def export_detailed_measurement_graphs(path: str) -> None:
    mss = get_measurements()
    groups = get_object_side_groups(mss)
    build_detailed_graphs_and_save(groups, path)
